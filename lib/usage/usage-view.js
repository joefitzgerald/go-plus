...
export default class UsageView extends EtchComponent {
  constructor (props) {
    ...
    if (props.model) {
      ...
      props.model.updateContent('To find usage, select an identifier and run the `golang:find-usage` command via the command pallette.', 'initial')
    }
  }

  defaultContent () {
    return 'To find usage, select an identifier and run a the `golang:find-usages` command via the command pallette.'
  }
