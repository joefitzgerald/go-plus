...
const defaultContent =
  'To find usage, select an identifier and run the `golang:find-usages` command via the command palette.'

export default class UsageView extends EtchComponent {
  props: {
    content?:
      | string
      | { raw: string, referrers: Map<string, Array<Reference>> },
    model?: any,
    style?: string
  }
          ...
          return (
            <details className="go-plus-accordion-item" open>
              <summary className="go-plus-accordion-header">
                <Octicon name="package" /> {pkg}{' '}
                <span className="badge badge-info">{refs.length}</span>
              </summary>
              <main className="go-plus-accordion-content">
                <table className="go-plus-table">
                  {refs.map(ref => {
                    ...
                    return (
                      <tr
                        onClick={this.handleClick.bind(this, ref)}
                        className="go-plus-table-row"
                      >
                        <td className="go-plus-table-cell">
                          {ref.text.trim()}{' '}
                          <span className="text-subtle">
                            at {filename}:{ref.row}:{ref.column}
                          </span>
                        </td>
                      </tr>
                    )
