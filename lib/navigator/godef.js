...
class Godef {
  ...
  async gotoDefinitionForBufferPosition (pos: any, editor: any = getEditor()): Promise<any> {
    if (!editor || !pos) {
      return false
    }
    try {
      ...
      if (stderr && stderr.trim() !== '') {
        console.log(tool, '(stderr) ' + stderr) // eslint-disable-line no-console
      }
  ...
  highlightWordAtCursor (editor: any): Disposable {
    ...
    return cursor.onDidChangePosition(onDidChange)
  }
}
