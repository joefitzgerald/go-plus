...
class Godef {
  ...
  gotoDefinitionForBufferPosition (pos: [number, number], editor: any = getEditor()): Promise<any> {
    if (!editor || !pos) {
      return Promise.resolve(false)
    }
    const tool = atom.config.get('go-plus.navigator.mode')
    const prom = tool === 'guru'
      ? this.gotoDefinitionGuru(pos, editor)
      : this.gotoDefinitionGodef(pos, editor)
  ...
  gotoDefinitionGuru (pos: [number, number], editor: any): Promise<ExecResult | false> {
    if (!editor || !pos) {
      return Promise.resolve(false)
    }
  ...
  gotoDefinitionGodef (pos: [number, number], editor: any): Promise<ExecResult | false> {
  ...
  visitLocation (loc: DefLocation | null) {
    if (!loc || !loc.filepath) {
      const opts = {}
    ...
    const path: string = loc.filepath
  ...
  visitFile (loc: DefLocation): Promise<void> {
    return openFile(loc.filepath, loc.pos).then((editor) => {
      if (loc.pos) {
  ...
  visitDirectory (loc: DefLocation): Promise<void> {
