...
type WillSaveCallback = any => boolean
...
class Orchestrator {
    ...
    this.subscriptions.add(
      atom.workspace.observeTextEditors(editor => {
        ...
        const editorDidSaveSubscription = editor.onDidSave(evt => {
          this.editorDidSave(editor, evt.path).catch(error => {
  ...
  editorDidSave(editor: any, path: string): Promise<void> {
    return Array.from(this.didSaveCallbacks.entries()).reduce(
      (p, [, func]) => p.then(() => func(editor, path)),
      Promise.resolve()
    )
  }
}
