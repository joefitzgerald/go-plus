...
class GocodeProvider implements AutocompleteProvider {
  ...
  getSuggestions (options: SuggestionRequest): Promise<Array<Suggestion>> {
    ...
    // TODO: Re-implement caching of suggestions once we track buffer position
    // more accurately, reset the cache when position changes occur
    // if (prefix.length > 0 && !prefix.endsWith('.') && this.currentSuggestions.length) {
