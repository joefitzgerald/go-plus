...
class GocodeProvider implements AutocompleteProvider {
  ...
  toggleGocodeConfig () {
    if (this.goconfig) {
      this.goconfig.locator.findTool('gocode').then((cmd) => {
        ...
        const gocode = cmd
        this.goconfig.executor.exec(gocode, ['set', 'unimported-packages', this.unimportedPackages.toString()]).then((r) => {
          ...
          return this.goconfig.executor.exec(gocode, ['set', 'propose-builtins', this.proposeBuiltins.toString()]).then((r) => {
            const stderr = r.stderr instanceof Buffer ? r.stderr.toString() : r.stderr
            if (stderr && stderr.trim() !== '') {
              console.log('autocomplete-go: (stderr) ' + stderr)
            }
  ...
  getSuggestions (options: SuggestionRequest): Promise<Array<Suggestion>> {
    ...
    const p: Promise<Array<Suggestion>> = new Promise((resolve) => {
      ...
      this.goconfig.locator.findTool('gocode').then((cmd) => {
        ...
        const args = ['-f=json', 'autocomplete', buffer.getPath(), offset.toString()]
        ...
        this.goconfig.executor.exec(cmd, args, execOptions).then((r) => {
          ...
          if (stdout && stdout.trim() !== '') {
            messages = this.mapMessages(stdout, options.editor, options.bufferPosition)
          }
  ...
  mapMessages (data: string, editor: any, position: any): Array<Suggestion> {
    if (!data) {
      return []
    }
    let res
    ...
    const candidates: Array<GoCodeSuggestion> = res[1]
    if (!candidates) {
      return []
    }
