...
export function getRelativePackagePath(
  currentDir: string,
  ...
  if (vendorIndex > -1) {
    let rootProjectForVendorPkg = path.join(
      currentWorkspace,
      pkg.substr(0, vendorIndex)
    )
    let relativePathForVendorPkg = pkg.substring(
      vendorIndex + magicVendorString.length
    )

    if (
      relativePathForVendorPkg &&
      currentDir.startsWith(rootProjectForVendorPkg)
    ) {
