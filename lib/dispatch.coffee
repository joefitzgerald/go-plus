...
class Dispatch
  ...
  ##
  # combineMessages takes an array of message arrays (nested 1 deep) and
  # flattens and eliminates duplicates.
  #
  # Optionally it merges messages with a prior list of messages, deduping based
  # on line, column and message.
  combineMessages: (messages, combineWith = undefined) ->
    messages ?= []
    messages = _.flatten(messages, true) if messages.length > 0
    messages = _.filter(messages, (element) -> element?)
    if combineWith
      messages = _.union(combineWith, messages)
    _.uniq messages, (element, index, list) ->
      return element?.line + ':' + element?.column + ':' + element?.msg

  collectMessages: (messages) ->
    @messages = @combineMessages(messages, @messages)
    @emit('messages-collected', _.size(@messages))
    @messages

  ##
  # Runs gofmt and other configured go tools on the supplied editor, returning
  # a promise that is fulfilled with an array of error/warning messages
  # generated by the tools.
  applyGoTools: (editor, saving = true) ->
    messages = []
    new Promise (resolve, reject) =>
      async.series([
        (callback) =>
          @gofmt.formatBuffer(editor, saving, callback)
      ], (fmterr, modifymessages) =>
        messages = modifymessages and @combineMessages(modifymessages)
        async.parallel([
          (callback) =>
          ...
          if err or fmterr
            reject(err or fmterr)
          else
            resolve(@collectMessages(@combineMessages(checkmessages, messages)))
        )
      )
  ...
  triggerPipeline: (editor, saving) ->
    ...
    # If we're acting as an Atom-Linter provider, the linter will call us, so
    # don't run the tools independently:
    if @isAtomLinterActive()
      @emit('dispatch-complete', editor)
    else
      @applyGoTools(editor, saving).then (messages) =>
        @collectMessages(messages)
        @emit('dispatch-complete', editor)
