...
class Tester {
  ...
  createCoverageFile () {
    ...
    if (!this.tempDir) {
      var affix = {}
      var customDir = atom.config.get('go-plus.test.customTestDirectory') || ''

      if(customDir.length > 0){
        affix.dir = customDir
      }

      this.tempDir = fs.realpathSync(temp.mkdirSync(affix))
    }
  ...
  runTests (editor = getEditor()) {
    ...
    return Promise.resolve().then(() => {
      ...
      try{
        ...
        this.testPanelManager.update({output: `failed to create coverage file, please check your test directory settings [currently: ${atom.config.get('go-plus.test.customTestDirectory')}]`, status: 'fail', exitcode: 0 })
        this.running = false
        return false
      }
