...
export default class GoPlusPanel extends EtchComponent {
  ...
  render () {
    ...
    return (
      <atom-panel ref='thepanel' className={panelClass}>
        <div className='panel-heading'>
          <nav className='panel-group panel-nav'>
            {tabs.map((item) => {
              const tabKey = item.key + '-tab'
              return <span key={tabKey} className={item.className} on={{click: () => this.handleTabClick(item)}}><Octicon name={item.icon} /> <span className='panel-nav-label'>{item.name}</span></span>
            })}
          </nav>
        </div>
