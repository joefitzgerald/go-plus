...
export default class GoPlusPanel extends EtchComponent {
  ...
  render () {
    ...
    // TODO: update tester-go and godoc to use the below approach too
    let tabs = [
      {key: 'reference', packageName: 'godoc', name: 'Reference', icon: 'book', order: 300}
    ]

    // get additional tabs from the view providers
    this.props.viewProviders.forEach(({ model }) => {
      if (tabs.find(({ key }) => key === model.key)) {
        return
      }
      tabs.push(Object.assign({ key: model.key, order: 999 }, model.tab))
    })
