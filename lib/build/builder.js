...
class Builder {
  ...
  lintInstall (cmd, options) {
    ...
    const buildTags = atom.config.get('go-plus.build.tags')
    if (buildTags && buildTags.length) {
      buildArgs.push('--tags', '"' + buildTags + '"')
    }
    buildArgs.push('.')
  ...
  lintTest (cmd, options) {
    ...
    const buildTags = atom.config.get('go-plus.build.tags')
    if (buildTags && buildTags.length) {
      testArgs.push('--tags', '"' + buildTags + '"')
    }
    testArgs.push('-c', '-o', tempdir, '.')
