...
class Builder {
  ...
  getMessages(
    results: Array<{ output: string, linterName: string }>,
    cwd: string
    ...
    for (const { output, linterName } of results) {
      ...
      for (const newMessage of newMessages) {
        if (
          !messages.some(message => this.messageEquals(newMessage, message))
        ) {
  ...
  async lintInstall(cmd: string, options: ExecutorOptions) {
    options.timeout = (atom.config.get('go-plus.config.buildTimeout'): any) || 10000
  ...
  async lintTest(cmd: string, options: ExecutorOptions) {
    ...
    const testArgs = this.testCompileArgs(additionalArgs)
