...
class Locator {
  ...
  constructor(options?: { executor?: Executor }) {
    ...
    this.goLocators = [
      () => this.gorootLocator(), // check $GOROOT/bin
      () => this.configLocator(), // check Atom configuration
      () => this.pathLocator(), // check $PATH
      () => this.defaultLocator() // check common installation locations
    ]
  ...
  runtimes(): Promise<Array<Runtime>> {
    ...
    return new Promise(resolve => {
      ...
      for (const candidate of candidates) {
        const goversion = this.executor.execSync(candidate, ['version'], {
      ...
      for (const viableCandidate of viableCandidates) {
        ...
        const stdout =
          goenv.stdout instanceof Buffer
            ? goenv.stdout.toString()
            : goenv.stdout
  ...
  runtimeCandidates() {
    ...
    for (const locator of this.goLocators) {
      ...
      if (Array.isArray(c) && c.length > 0) {
        // take the union of candidates and c, removing any duplicates
        candidates = [...new Set([...candidates, ...c])]
      }
    }
    return candidates
  }
