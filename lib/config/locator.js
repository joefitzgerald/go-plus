...
class Locator {
  ...
  runtimes (): Promise<Array<Runtime>> {
    ...
    return new Promise((resolve) => {
      ...
      for (const viableCandidate of viableCandidates) {
        const goenv = this.executor.execSync(viableCandidate.path, ['env', '-json'], { cwd: path.dirname(viableCandidate.path) })
  ...
  statishSync (pathValue: string) {
    ...
    if (pathValue && pathValue.trim() !== '') {
      try { stat = fs.statSync(pathValue) } catch (e) { } // eslint-disable-line no-empty
    }
    return stat
  }
  ...
  configLocator (): Array<string> {
    const goinstallation = (atom.config.get('go-plus.config.goinstallation'): any)
