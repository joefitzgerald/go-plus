  ...
  exec(
    ...
    return new Promise(resolve => {
      ...
      if (options.timeout && options.timeout > 0) {
        setTimeout(() => {
          // Note: it is not strictly necessary to kill the process here,
          // as Atom's BufferedProcess does pass the timeout to node's
          // child_process module.  We do this so that we can explicitly
          // set the exitcode in order to allow downstream code to detect
          // a timeout.
          //
          // Also note: this code will run every time (there is nothing that
          // cancels the timeout), so we can't assume that if we got here
          // the process actually timed out.  (This works because resolve()
          // is effectively a no-op if the promise was already resolved.)
          //
          // TODO: try to detect timeouts here instead of at each exec call.
          // [this will require canceling the timeout..]
