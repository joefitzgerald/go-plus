...
class Gorename {
  ...
  commandInvoked () {
    ...
    this.goconfig.locator.findTool('gorename').then((cmd: FindResult) => {
      if (!cmd) {
        return
      }
      ...
      const dialog = new SimpleDialog({
        ...
        onConfirm: (newName) => {
          this.saveAllEditors().then(() => {
            ...
            this.runGorename(file, offset, cwd, newName, c)
          })
        },
        onCancel: () => {
          editor.setCursorBufferPosition(cursor, {autoscroll: false})
  ...
  runGorename (file: string, offset: number, cwd: string, newName: string, cmd: string) {
