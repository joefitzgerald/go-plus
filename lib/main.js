...
export default {
  ...
  activate () {
    ...
    const SaveEventOrchestrator = require('./orchestrator')
    this.saveEvent = new SaveEventOrchestrator()
    this.subscriptions.add(this.saveEvent)
  ...
  loadFormatter () {
    ...
    this.subscriptions.add(this.saveEvent.onWillSave((e) => { return this.formatter.handleWillSaveEvent(e) }))
  ...
  loadTester () {
    ...
    this.subscriptions.add(this.saveEvent.onDidSave('tester', (editor, path) => { return this.tester.handleSaveEvent(editor, path) }))
    return this.tester
  },
  ...
  loadBuilder (linter) {
    ...
    this.builder = new Builder(this.provideGoConfig(), linter)

    if (this.subscriptions) {
      this.subscriptions.add(this.builder)
    }

    this.subscriptions.add(this.saveEvent.onDidSave('builder', (editor, path) => { return this.builder.build(editor, path) }))
    return this.builder
  },
  ...
  loadGometalinter (linter) {
    ...
    this.linter = new GometalinterLinter(this.provideGoConfig(), linter)

    if (this.subscriptions) {
      this.subscriptions.add(this.linter)
    }

    this.subscriptions.add(this.saveEvent.onDidSave('linter', (editor, path) => { return this.linter.lint(editor, path) }))
    return this.linter
  },
  ...
  consumeIndieLinterV1 (registry) {
    const buildLinter = registry.register({name: 'go build'})
    this.subscriptions.add(buildLinter)
    this.loadBuilder(buildLinter)

    // make sure tests are run before linting
    // TODO(zb): move all save event registrations to one place so that order is clear
    this.loadTester()

    const gometalinterLinter = registry.register({name: 'gometalinter'})
    this.subscriptions.add(gometalinterLinter)
    this.loadGometalinter(gometalinterLinter)
  },
