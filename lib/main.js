  ...
  load() {
    ...
    this.subscriptions.add(
      this.orchestrator.register(
        'format',
        e => {
          return this.formatter.handleWillSaveEvent(e)
        },
        'willSave'
      )
    )
    this.subscriptions.add(
      this.orchestrator.register('builder', (editor, path) => {
        return this.builder.build(editor, path)
      })
    )
    this.subscriptions.add(
      this.orchestrator.register('tester', (editor, path) => {
        return this.tester.handleSaveEvent(editor, path)
      })
    )
    this.subscriptions.add(
      this.orchestrator.register('linter', (editor, path) => {
        return this.linter.lint(editor, path)
      })
    )
