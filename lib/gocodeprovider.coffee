...
class GocodeProvider
  ...
  getSuggestions: (options) ->
    return new Promise((resolve) =>
      ...
      @callGocode(buffer.getPath(), offset, text)
        .then(
          (stdout) ->
            messages = provider.mapMessages(stdout, options.editor, options.bufferPosition) if stdout? and stdout.trim() isnt ''
  ...
  callGocode: (editorPath, offset, text) ->
    return new Promise((resolve, reject) =>
      go = @dispatch.goexecutable.current()
      return reject() unless go?
      gopath = go.buildgopath()
      return reject() if not gopath? or gopath is ''

      env = @dispatch.env()
      env['GOPATH'] = gopath
      cwd = path.dirname(editorPath)
      args = ['-f=json', 'autocomplete', editorPath, offset]
      ...
      if cmd is false
        ...
        return reject(@name + ' - stderr: ' + stderr) if stderr? and stderr.trim() isnt ''
        resolve(stdout.trim())
