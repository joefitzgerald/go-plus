...
export default class GoModifyTags {
  ...
  commandInvoked (mode: Mode) {
    ...
    return this.goconfig.locator.findTool('gomodifytags').then((cmd: FindResult) => {
      if (cmd) {
  ...
  modifyTags (editor: any, options: GoModifyTagsOptions, mode: Mode, cmd: string) {
