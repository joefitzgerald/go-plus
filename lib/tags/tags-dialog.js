...
export class TagsDialog extends EtchComponent {
  tags: Array<Tag>
  ...
  constructor(props: Object) {
    ...
    this.subscriptions.add(
      atom.commands.add(this.element, 'core:cancel', () => {
        this.cancel()
      })
    )
    this.subscriptions.add(
    ...
    this.subscriptions.add(
      this.refs.tag.onDidStopChanging(() =>
        this.checkInput(this.refs.tag, this.tagRegex)
      )
    )
    this.subscriptions.add(
      this.refs.option.onDidStopChanging(() =>
        this.checkInput(this.refs.option, this.optionRegex)
      )
    )
  ...
  render() {
    const icon =
      this.props.mode === 'Add' ? 'icon icon-tag-add' : 'icon icon-tag-remove'
    const tags =
      this.tags && this.tags.length ? this.tags.map(this.makeListItem) : null
    ...
    return (
      <div className="gomodifytags">
        <h1>
          <span className={icon}>{this.props.mode} Tags</span>
        </h1>
        <div className="go-tags-flex-container">
          <div className="tag">
            <TextEditor mini ref="tag" placeholderText="tag (eg. json)" />
          </div>
  ...
  attach() {
    this.subscriptions.add(
      atom.commands.add(this.element, 'gomodifytags:focus-next', () =>
        this.focusNextElement(1)
      )
    )
    this.subscriptions.add(
      atom.commands.add(this.element, 'gomodifytags:focus-previous', () =>
        this.focusNextElement(-1)
      )
    )
  ...
  focusNextElement(direction: number) {
    const elements: Array<any> = [
      this.refs.tag.element,
      this.refs.option.element,
      this.refs.submitButton
    ]
    const focusedElement = elements.find((el: any) =>
