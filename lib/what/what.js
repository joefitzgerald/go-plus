...
class What {
  ...
  async run(editor: any, pos: any, cursor: any, shouldDecorate: boolean) {
    if (this.running) {
      return
    }
    ...
    if (!cmd) {
      console.log('missing guru tool') // eslint-disable-line no-console
      return
    }
    if (!this.goconfig) {
      return
    ...
    if (shouldDecorate && result) {
      if (
        result.enclosing &&
        result.enclosing.length &&
        result.sameids &&
        result.sameids.length
      ) {
        let length = 0
        ...
        for (const sameid of result.sameids) {
          ...
          if (
            ...
            this.currentEditorLayer.markBufferRange(
              [start, [start[0], start[1] + length]],
              { invalidate: 'touch' }
            )
          }
        }
        editor.decorateMarkerLayer(this.currentEditorLayer, {
          type: 'highlight',
          class: 'sameid',
          onlyNonEmpty: true
        })
