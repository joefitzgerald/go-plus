...
const toOutline = (
  ...
  goOutline.forEach(item => {
    ...
    if (label === '_' && item.type === 'variable') return
    ...
    if (item.type === 'package') item.type = 'file'
    if (item.type === 'import') item.type = 'package'
    ...
    if (item.type === 'type') {
      ...
      if (line.includes('type')) {
        if (line.includes('interface')) item.type = 'interface'
        if (line.includes('struct')) item.type = 'class' // TODO: is there a better icon?
      }
    ...
    if (item.type === 'variable' && line.includes('const')) {
      item.type = 'constant'
    }
